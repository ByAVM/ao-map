...
watch(() => state.current,  (icon) => emit('change', icon))
</script>

<template>
    <base-select v-model="state.current" :items="state.icons" keyValue="type">
        <template #selected="{item}">
            <div class="icon-select__item icon-select__item__selected">
                ...
                <span v-text="item.title" class="icon-select__item-text"></span>
            </div>
        </template>
...
.icon-select__item__selected {}
.icon-select__item-image {
    width: 32px;
    display: inline-block;
    margin-right: 4px;
}
.icon-select__item-text {}
</style>
